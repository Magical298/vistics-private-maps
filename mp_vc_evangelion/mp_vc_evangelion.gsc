...
bouncegun_room()
{
    level.bgun = getEnt( "bgunroom", "targetname");
    jump = getEnt( "jumper_bgun", "targetname" );
    acti = getEnt( "acti_bgun", "targetname" );

    
    while(1)
    {
        level.bgun waittill( "trigger", player );
        if( !isDefined( level.bgun ) )
            return;
		...
		thread roomhud(0,-100,"Bounce Gun Room");
		thread roomhud(-100,-50,player.name);
		thread roomhud(0,-50,"^1VS");
        ...
        player setOrigin( jump.origin );
		player TakeAllWeapons();
		player GiveWeapon( "bgun_mp" );
		player givemaxammo("bgun_mp");
		player thread firestarter();

        level.activ setPlayerangles( acti.angles );
        level.activ setOrigin( acti.origin );
		level.activ TakeAllWeapons();
		level.activ GiveWeapon( "bgun_mp" );
		level.activ givemaxammo("bgun_mp");
		level.activ thread firestarter();

	    player freezeControls(true);
		level.activ freezeControls(true);

		player switchToWeapon( "bgun_mp" );
		level.activ SwitchToWeapon( "bgun_mp" );
		wait 5;
	    player freezeControls(false);
		level.activ freezeControls(false);
        while( isAlive( player ) && isDefined( player ) )
...
vc_logorotate()
{
	logo = getent("vc_logo","targetname");
	for(;;)
	{
...
bouncescript()
{
	if(self getCurrentWeapon() == "bgun_mp")
		{
			vec = anglestoforward(self getPlayerAngles());
			end = (vec[0] * 200000, vec[1] * 200000, vec[2] * 200000);
			location = BulletTrace( self gettagorigin("tag_eye"), self gettagorigin("tag_eye")+end, 0, self)[ "position" ];
			showbounce = spawn("script_model",location);
			showbounce setmodel("wraith");
			trig = spawn("trigger_radius",location,0,35,80);
			for(i=0;i<1;i++)
			{
				trig waittill("trigger",player);
				player thread player_bounce(trig);
			}
				wait .5;
				trig delete();
				showbounce delete();
